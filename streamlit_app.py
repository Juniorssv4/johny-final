...
def save_glossary_to_github(glossary_dict):
    try:
        ...
        if put_response.status_code in (200, 201):
            st.success("New term saved to GitHub! Changes are permanent.")
...
with st.expander("➕ Teach Johny a new term (add in app & save to GitHub)"):
    ...
    if st.button("Add New Term"):
        if new_eng and new_lao:
            eng_lower = new_eng.strip().lower()
            glossary[eng_lower] = new_lao.strip()
            # Save to GitHub
            ...
            data = {
                "message": "Add new term from app",
                "content": encoded,
                "sha": sha
            }
            put_response = requests.put(url, headers=headers, json=data)
            if put_response.status_code in (200, 201):
                st.success(f"Added: {new_eng} → {new_lao}")
                st.rerun()
            else:
                st.error(f"GitHub save failed: {put_response.text}")
        else:
